#!/usr/bin/env bash
# Phase 3: Behavioral Validation Script
set -euo pipefail

BINARY="$HOME/deepseek-tui-fork/target/release/deepseek-tui"
PRESETS="safe create explore debug director partner"
PORT=9799
AUTH_TOKEN="phase3-token-$(date +%s)"
...
for preset in $PRESETS; do
    ...
    PROMPT_FILE="/tmp/phase3-${preset}-prompt-clean.txt"
    RESPONSE_FILE="/tmp/phase3-${preset}-response.txt"
    ...
    THREAD_OUT=$(curl -sf -X POST "$BASE_URL/v1/threads" \
        ...
        -d '{"system_prompt": '"$SP"', "workspace": "$HOME/deepseek-tui-modes", "auto_approve": true, "mode": "agent"}' 2>&1) || {
    ...
    echo "$TEST_TASK" > /tmp/phase3-task-body.json
    TURN_OUT=$(curl -sf -X POST "$BASE_URL/v1/threads/$THREAD_ID/turns" \
        -H "Authorization: Bearer $AUTH_TOKEN" \
        -H "Content-Type: application/json" \
        -d @/tmp/phase3-task-body.json 2>&1) || {
    ...
    while true; do
        ...
        echo "$THREAD_STATE" > /tmp/phase3-${preset}-thread-state.json
...
")
    ...
    if [ "$TURN_STATUS" = "completed" ]; then
        ...
        python3 -c "
import sys,json
d=json.load(open('/tmp/phase3-${preset}-thread-state.json'))
for t in d.get('turns',[]):
    if t.get('id')=='$TURN_ID':
        ...
        for item in items:
            ...
            if txt:
                print(txt)
        if not items:
            # print the turn structure for debugging
...
for preset in $PRESETS; do
    RF="/tmp/phase3-${preset}-response.txt"
    if [ -f "$RF" ]; then
